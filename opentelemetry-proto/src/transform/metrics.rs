...
use super::{common, proto};
use std::fmt::Debug;

use opentelemetry::{otel_debug, Key, Value};
use opentelemetry_sdk::metrics::data::{
    AggregatedMetrics, Exemplar as SdkExemplar, ExponentialHistogram as SdkExponentialHistogram,
    Gauge as SdkGauge, Histogram as SdkHistogram, Metric as SdkMetric, MetricData, ResourceMetrics,
...
use proto::{
    ...
    metrics::v1::{
        ...
        exponential_histogram_data_point::Buckets as TonicBuckets, metric::Data as TonicMetricData,
        ...
        ExponentialHistogramDataPoint as TonicExponentialHistogramDataPoint, Gauge as TonicGauge,
        ...
        TonicHistogram {
            data_points: hist
                ...
                .map(|dp| TonicHistogramDataPoint {
                    ...
                    flags: TonicDataPointFlags::DoNotUse as u32,
                    min: dp.min().map(Numeric::into_f64),
                    max: dp.max().map(Numeric::into_f64),
                })
                .collect(),
            aggregation_temporality: TonicTemporality::from(hist.temporality()) as i32,
        }
    }
}
    ...
    fn from(sum: &SdkSum<T>) -> Self {
        TonicSum {
            data_points: sum
                ...
                .collect(),
            aggregation_temporality: TonicTemporality::from(sum.temporality()) as i32,

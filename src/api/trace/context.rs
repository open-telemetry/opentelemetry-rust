...
pub trait TraceContextExt {
    ...
    /// assert_eq!(Context::current().span().span_context(), api::trace::SpanReference::empty_context());
    ///
    ...
    ///     assert_ne!(cx.span().span_context(), api::trace::SpanReference::empty_context());
    /// });
    /// ```
    ...
    fn with_remote_span_context(&self, span_context: api::trace::SpanReference) -> Self;

    /// Returns a reference to the remote span context data stored in this context,
    /// or none if no remote span context has been set.
    ///
    /// This is useful for building tracers.
    fn remote_span_context(&self) -> Option<&api::trace::SpanReference>;
}
...
impl TraceContextExt for api::Context {
    ...
    fn with_remote_span_context(&self, span_context: api::trace::SpanReference) -> Self {
        self.with_value(RemoteSpanReference(span_context))
    }

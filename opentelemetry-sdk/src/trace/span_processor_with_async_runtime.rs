...
impl<R: RuntimeChannel> SpanProcessor for BatchSpanProcessor<R> {
    ...
    fn on_end(&self, span: SpanData) {
        ...
        if result.is_err() {
            // Increment the number of dropped spans and invoke error handler
...
impl<E: SpanExporter + 'static, R: RuntimeChannel> BatchSpanProcessorInternal<E, R> {
    async fn flush(&mut self, res_channel: Option<oneshot::Sender<OTelSdkResult>>) {
        ...
        if let Some(channel) = res_channel {
            // For user-initiated flush, send the result through the response channel
            let _ = channel.send(export_result);
        } else {
            // For timer-based/automatic flushes, check if export failed
            if let Err(err) = export_result {
                // Background export failed. Invoke error handler if registered.
                if let Some(ref handler) = self.error_handler {
                    handler(err);
                }
            }
        ...
        while let Some(result) = self.export_tasks.next().await {
            // Check results from concurrent exports
            if let Err(err) = result {
                // Concurrent export task failed. Invoke error handler if registered.
                if let Some(ref handler) = self.error_handler {
                    handler(err);
                }
            }
                ...
                if self.spans.len() == self.config.max_export_batch_size {
                        ...
                        && self.export_tasks.len() == self.config.max_concurrent_exports
                    {
                        if let Some(result) = self.export_tasks.next().await {
                            if let Err(err) = result {
                                // Background export task failed. Invoke error handler if registered.
                                if let Some(ref handler) = self.error_handler {
                                    handler(err);
                                }
                            }
                    ...
                    if self.config.max_concurrent_exports == 1 {
                        if let Err(err) = task.await {
                            // Synchronous export failed during batch processing. Invoke error handler.

...
impl Runtime for Tokio {
        ...
        T: Send + 'static,
    {
        ...
        if let Ok(handle) = tokio::runtime::Handle::try_current() {
            ...
            //
            // `num_workers()` is a snapshot taken at spawn time: tokio's
            // multi_thread runtime does not add or remove worker threads
            // after startup, so this check cannot go stale for the lifetime
            // of a given runtime. It does not, however, protect against a
            // runtime that later becomes saturated by other work — it only
            // tells us how many workers exist, not how busy they are.

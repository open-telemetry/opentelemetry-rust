...
/// Runtime implementation that works with Tokio.
///
/// Automatically detects the current runtime flavor and handles both
/// multi-threaded and current-thread runtimes:
/// - **Multi-threaded runtime**: Spawns tasks directly on the tokio runtime.
/// - **Current-thread runtime**: Spawns a separate OS thread with its own
///   tokio current-thread runtime to avoid deadlocks.
/// - **No runtime available**: Spawns a separate OS thread with a new
            ...
            Ok(handle) => {
                match handle.runtime_flavor() {
                    tokio::runtime::RuntimeFlavor::CurrentThread => {
                        // Single-threaded: spawn separate OS thread with own runtime

...
/// Configuration options for [PeriodicReaderWithOwnThread].
///
...
/// attempts. PeriodicReaderWithOwnThread itself does not enforce timeout. Instead timeout
...
#[derive(Debug)]
pub struct PeriodicReaderWithOwnThreadBuilder<E> {
    interval: Duration,
    timeout: Duration,
    exporter: E,
}

impl<E> PeriodicReaderWithOwnThreadBuilder<E>
where
    E: PushMetricExporter,
{
    fn new(exporter: E) -> Self {
        ...
        PeriodicReaderWithOwnThreadBuilder {
            interval,
            timeout,
            exporter,
        }
    ...
    /// Configures the intervening time between exports for a [PeriodicReaderWithOwnThread].
    ///
    ...
    /// Configures the timeout for an export to complete. PeriodicReaderWithOwnThread itself
    ...
    /// Create a [PeriodicReaderWithOwnThread] with the given config.
    pub fn build(self) -> PeriodicReaderWithOwnThread {
        PeriodicReaderWithOwnThread::new(self.exporter, self.interval, self.timeout)
    }
}
...
/// interval.
///
/// By default, PeriodicReaderWithOwnThread will collect and export data every
/// 60 seconds. The export time is not counted towards the interval between
...
///
/// let reader = PeriodicReaderWithOwnThread::builder(exporter).build();
/// # drop(reader);
/// # }
/// ```
#[derive(Clone)]
pub struct PeriodicReaderWithOwnThread {
    inner: Arc<PeriodicReaderInner>,
}

impl PeriodicReaderWithOwnThread {
    /// Configuration options for a periodic reader with own thread
    pub fn builder<E>(exporter: E) -> PeriodicReaderWithOwnThreadBuilder<E>
    where
        E: PushMetricExporter,
    {
        PeriodicReaderWithOwnThreadBuilder::new(exporter)
    }
        ...
        E: PushMetricExporter,
    {
        let (message_sender, message_receiver): (Sender<Message>, Receiver<Message>) =
            mpsc::channel();
        let reader = PeriodicReaderWithOwnThread {
        ...
        let result_thread_creation = thread::Builder::new()
            .name("OpenTelemetry.Metrics.PeriodicReaderWithOwnThread".to_string())
            .spawn(move || {
...
impl fmt::Debug for PeriodicReaderWithOwnThread {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PeriodicReaderWithOwnThread").finish()
    }
}
...
mod tests {
    ...
    fn collection_triggered_by_interval_multiple() {
        ...
        let reader = PeriodicReaderWithOwnThread::builder(exporter.clone())

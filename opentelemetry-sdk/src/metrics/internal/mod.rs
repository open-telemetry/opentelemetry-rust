    ...
    T: Number,
{
    ...
    fn update(&self, config: &Self::Config, measurement: T);
}
    ...
    A: Aggregator<T>,
{
    ...
    fn measure(&self, measurement: T, attributes: &[KeyValue]) {
        if attributes.is_empty() {
            self.no_attribute_tracker.update(&self.config, measurement);
        ...
        if let Some(tracker) = trackers.get(attributes) {
            tracker.update(&self.config, measurement);
            return;
        }
        ...
        if let Some(tracker) = trackers.get(attributes) {
            ...
            new_tracker.update(&self.config, measurement);
            ...
            overflow_value.update(&self.config, measurement);
        } else {

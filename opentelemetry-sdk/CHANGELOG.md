...
- `BatchSpanProcessor::new()` and `BatchLogProcessor::new()` now return `Err(ProviderBuildError::InternalFailure)` if the exporter requires an async runtime (e.g. `reqwest::Client`, `HyperClient`) but the batch processor uses a synchronous OS thread. Previously, this combination would panic at export time with "no reactor running". `SpanExporter` and `LogExporter` gain a new `requires_async_runtime() -> bool` method (default: `false`) to declare this requirement.

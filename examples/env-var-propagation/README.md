...
2. The parent injects that span context into an `EnvVarInjector`, then passes
   the resulting environment variables to a child process.
3. The child process captures its startup environment with `EnvVarExtractor`,
   extracts the propagated context, and starts its own span as a child of the
   parent's span.
